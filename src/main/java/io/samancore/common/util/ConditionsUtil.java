...
public class ConditionsUtil {
    ...
    public static Map<String, Object> getModelMapUpdatedWithConditions(Map<String, Object> modelMap, List<Condition> conditions) {
        var validateError = conditions.stream().filter(condition -> condition.getConditionType().equals(ConditionType.VALIDATE) && condition.getValue() != null && !StringUtils.isEmpty(condition.getValue().toString())).toList();
